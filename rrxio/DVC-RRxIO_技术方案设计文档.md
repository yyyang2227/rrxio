...
如果原始 RRxIO 过度自信，通常会出现 $\rho_{\text{NIS}}\gg5\%$；如果重标定过度保守，则会出现 $\rho_{\text{NIS}}\ll5\%$，此时滤波器虽然“不容易超限”，但量测约束被削弱，可能无法改善局部 RPE 长尾。因此 NIS 不是越低越好，也不是越高越好。DVC-RRxIO 的目标是让该比例在全局和分组层面都接近理论一致性区间，同时不显著损失轨迹精度。

当前工程 Gate-W10 已采用 band 口径：默认检查 visual 雷达速度更新的 $\rho_{\text{NIS}}$ 是否处于 $[1\%,8\%]$ 并接近 $5\%$。该区间不是论文最终理论边界，而是小样本公开数据上的工程门禁带宽；论文中应报告每个序列的 $\rho_{\text{NIS}}$，避免全局均值掩盖“多数过保守、少数组过自信”的问题。
...
验收标准：雷达稀疏或几何退化场景中，NIS 超限率从过高或过低状态向理论一致性区间收敛；若原始系统已偏保守，则不能继续单纯压低 NIS。
...
- W9-W10（Contribution-3，visual-only）：保持 `BLOCKED`。
  ...
  - 对 W8 中间基线：`ATE degrade=-10.08%`，`NIS` 相对下降 `24.11%`，`runtime=-1.98%`，`committed_drop=-0.46%`；失败项为 `RPE degrade=5.53%` 与 `RPE95 improve=-5.82%`。
  - 对原始 W1 visual 基线：ATE `-1.95%`、RPE `-2.49%`、runtime `+0.22%`、committed drop `1.83%`，说明中位精度与总体实时性有收益；但 RPE95 `-2.32%`，未解决长尾。
  - NIS 一致性：全局超限率 `1.997%` 偏保守；`mocap_difficult/visual=11.51%` 偏过自信，其余多数 visual 组 `<1%` 偏保守。
- 已验证不宜作为全局修复的路径：
